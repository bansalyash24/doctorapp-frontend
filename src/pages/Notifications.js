...
function Notifications() {
  ...
  const markAllAsSeen=async()=>{
    try {
        ...
        const response = await axios.post(`${url}/api/user/mark-all-notifications-as-seen`, {userId : user._id} , {
            headers: {
  ...
  const deleteAll=async()=>{
    try {
        dispatch(showLoading());
        const response = await axios.post(`${url}/api/user/delete-all-notifications`, {userId : user._id} , {
            headers: {
                Authorization : `Bearer ${localStorage.getItem("token")}`
            }
        });
